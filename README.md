...
[Source](https://developer.apple.com/documentation/accelerate/training_a_neural_network_to_recognize_digits)
...
![A flow diagram that shows the flow of information during the training phase of the sample code project.](Documentation/flow_2x.png)

## Define the filter parameters

The sample creates a [`BNNSFilterParameters`](https://developer.apple.com/documentation/accelerate/bnnsfilterparameters) structure with [`useClientPointer`](https://developer.apple.com/documentation/accelerate/bnnsflags/2915304-useclientpointer). This flag instructs the layers to keep the provided pointers at creation time, and to work directly from that data rather than use internal copies of the data.
...
For each iteration of the training phase, the sample creates a matrix that represents a random digit, and a _one-hot_ encoded tensor of the same digit. The sample places digits randomly in a 20 x 20 matrix, so a `3` might appear in the matrix as the image below. This example renders `0` as `⚪️`, and `1` as `⚫️`.
...
![A graph with loss as the y-axis and training epochs as the x-axis that shows loss decreasing during training.](Documentation/loss_2x.png)
